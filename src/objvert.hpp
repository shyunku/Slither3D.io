...
ObjectVertexProperty create_sphere_vertex_property()
{
	...
	for (size_t i = 1; i < sphere_latitude_segment_num; i++)
	{
		// xy - z
		float lat_angle = PI / 2 - PI * (float)i / float(sphere_latitude_segment_num);
		float cl = cosf(lat_angle);
		float sl = sinf(lat_angle);
		for (size_t j = 0; j < sphere_longitude_segment_num; j++)
		{
			float long_angle = 2.f * PI * j / float(sphere_longitude_segment_num);
			...
			float rate_hor = (float)j / (float)(sphere_longitude_segment_num - 1);
			float rate_ver = 1 - (float)i / (float)(sphere_latitude_segment_num - 1);
	...
	for (size_t i = 0; i < sphere_longitude_segment_num - 1; i++)
	{
	...
	// end of index
	indices.push_back(upper_pole_index);
	indices.push_back(sphere_longitude_segment_num - 1);
	indices.push_back(0);
	triangle++;

	// medival vertices indexing
	uint buffer_index;
	for (size_t i = 1; i < sphere_latitude_segment_num - 1; i++)
	{
		for (size_t j = 0; j < sphere_longitude_segment_num - 1; j++)
		{
			buffer_index = sphere_longitude_segment_num * (i - 1) + j;
			// upper triangle
			indices.push_back(buffer_index);
			indices.push_back(buffer_index + sphere_longitude_segment_num);
			...
			indices.push_back(buffer_index + 1);
			indices.push_back(buffer_index + sphere_longitude_segment_num);
			indices.push_back(buffer_index + sphere_longitude_segment_num + 1);
			triangle++;
		}

		// end of index at each of ring
		uint horizontal_max_index = sphere_longitude_segment_num * i - 1;

		// upper triangle
		indices.push_back(horizontal_max_index);
		indices.push_back(horizontal_max_index + sphere_longitude_segment_num);
		indices.push_back(horizontal_max_index + 1 - sphere_longitude_segment_num);
		triangle++;
	...
	// end of index
	indices.push_back(lower_pole_index);
	indices.push_back(middle_vertex_final_index - sphere_longitude_segment_num + 1);
